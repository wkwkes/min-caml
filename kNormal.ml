...
let rec g env = function (* K正規化ルーチン本体 (caml2html: knormal_g) *)
  ...
  | Syntax.Not(_, e) -> g env (Syntax.If(Id.dmy_pos, e, Syntax.Bool(false), Syntax.Bool(true)))
  | Syntax.Neg(_, e) ->
    insert_let (g env e)
      (fun x -> Neg(x), Type.Int)
  | Syntax.Add(_, e1, e2) -> (* 足し算のK正規化 (caml2html: knormal_add) *)
    insert_let (g env e1)
  ...
  | Syntax.Eq _ | Syntax.LE _ as cmp ->
    g env (Syntax.If(Id.dmy_pos, cmp, Syntax.Bool(true), Syntax.Bool(false)))
  | Syntax.If(p, Syntax.Not(_, e1), e2, e3) -> g env (Syntax.If(p, e1, e3, e2)) (* notによる分岐を変換 (caml2html: knormal_not) *)
  | Syntax.If(_, Syntax.Eq(_, e1, e2), e3, e4) ->
    insert_let (g env e1)
      (fun x -> insert_let (g env e2)
          (fun y ->
             ...
             IfEq(x, y, e3', e4'), t3))
  | Syntax.If(_, Syntax.LE(_, e1, e2), e3, e4) ->
    insert_let (g env e1)
      (fun x -> insert_let (g env e2)
          (fun y ->
             let e3', t3 = g env e3 in
             let e4', t4 = g env e4 in
             IfLE(x, y, e3', e4'), t3))
  | Syntax.If(p, e1, e2, e3) -> g env (Syntax.If(p, Syntax.Eq(Id.dmy_pos, e1, Syntax.Bool(false)), e3, e2)) (* 比較のない分岐を変換 (caml2html: knormal_if) *)
  | Syntax.Let(_, (x, t), e1, e2) ->
  ...
  | Syntax.Tuple(es) ->
    ...
    bind [] [] es
  | Syntax.LetTuple(_, xts, e1, e2) ->
    insert_let (g env e1)
      (fun y ->
         let e2', t2 = g (M.add_list xts env) e2 in
         LetTuple(xts, y, e2'), t2)
  | Syntax.Array(_, e1, e2) ->
    insert_let (g env e1)
      (fun x ->
